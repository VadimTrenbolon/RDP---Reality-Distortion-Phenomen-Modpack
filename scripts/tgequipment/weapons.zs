#priority 400

import mods.techguns.GunStats;
import mods.techguns.ArmorStats;

GunStats.setWeaponStat("sawedoff","DAMAGE",6.0);
GunStats.setWeaponStat("combatshotgun","DAMAGE",4.5);

GunStats.setWeaponStat("goldenrevolver","DAMAGE",24.0);
GunStats.setWeaponStat("goldenrevolver","DAMAGE_MIN",12.0);

GunStats.setWeaponStat("boltaction","DAMAGE",18.0);
GunStats.setWeaponStat("boltaction","DAMAGE_MIN",12.0);

GunStats.setWeaponStat("flamethrower","DAMAGE",9.0);
GunStats.setWeaponStat("flamethrower","DAMAGE_MIN",4.5);

GunStats.setWeaponStat("rocketlauncher","DAMAGE",50.0);
GunStats.setWeaponStat("rocketlauncher","DAMAGE_MIN",50.0);
GunStats.setWeaponStat("rocketlauncher","GRAVITY",0.001);

GunStats.setWeaponStat("netherblaster","DAMAGE",16.0);
GunStats.setWeaponStat("netherblaster","DAMAGE_MIN",8.0);

GunStats.setWeaponStat("biogun","DAMAGE",14.0);
GunStats.setWeaponStat("biogun","DAMAGE_MIN",10.0);

GunStats.setWeaponStat("teslagun","DAMAGE",14.0);
GunStats.setWeaponStat("minigun","DAMAGE",8.0);
GunStats.setWeaponStat("lasergun","DAMAGE",18.0);

GunStats.setWeaponStat("blasterrifle","DAMAGE",28.0);
GunStats.setWeaponStat("blasterrifle","DAMAGE_MIN",21.0);

GunStats.setWeaponStat("scatterbeamrifle","DAMAGE",5.0);
GunStats.setWeaponStat("scatterbeamrifle","DAMAGE_MIN",5.0);

GunStats.setWeaponStat("sonicshotgun","DAMAGE",28.0);
GunStats.setWeaponStat("sonicshotgun","DAMAGE_MIN",15.0);

GunStats.setWeaponStat("mibgun","DAMAGE",32.0);
GunStats.setWeaponStat("mibgun","DAMAGE_MIN",24.0);
GunStats.setWeaponStat("alienblaster","DAMAGE",36.0);
GunStats.setWeaponStat("alienblaster","DAMAGE_MIN",18.0);

GunStats.setWeaponStat("nucleardeathray","DAMAGE",9.0);
GunStats.setWeaponStat("nucleardeathray","DAMAGE_MIN",5.0);

GunStats.setWeaponStat("tfg","DAMAGE",500.0);
GunStats.setWeaponStat("tfg","DAMAGE_MIN",140.0);
GunStats.setWeaponStat("tfg","BULLET_SPEED",0.9);

GunStats.setWeaponStat("laserpistol","DAMAGE",7.0);

GunStats.setWeaponStat("as50","DAMAGE",48.0);
GunStats.setWeaponStat("as50","DAMAGE_MIN",24.0);

GunStats.setWeaponStat("gaussrifle","DAMAGE",180.0);
GunStats.setWeaponStat("gaussrifle","DAMAGE_MIN",180.0);

GunStats.setWeaponStat("guidedmissilelauncher","DAMAGE",50.0);
GunStats.setWeaponStat("guidedmissilelauncher","DAMAGE_MIN",30.0);

GunStats.setWeaponStat("grimreaper","DAMAGE",50.0);
GunStats.setWeaponStat("grimreaper","DAMAGE_MIN",20.0);

GunStats.setWeaponStat("chainsaw","DAMAGE",15.0);
GunStats.setWeaponStat("chainsaw","DAMAGE_MIN",15.0);